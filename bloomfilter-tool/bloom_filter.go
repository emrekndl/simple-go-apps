...
func ImportFromJSON(filename string) (*BloomFilter, error) {
	...
	bf := NewBloomFilter(data.Size, data.NbHashes)

	for i := 0; i < data.Size; i++ {
		byteIndex := i / 8
		bitIndex := uint(i % 8)
		if byteIndex < len(data.Bitset.Data) {
			bf.bitset[i] = (data.Bitset.Data[byteIndex] & (1 << bitIndex)) != 0
		}
	}
